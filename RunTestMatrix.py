...
def find_row(rows, row_id):
    for r in rows:
        rid = clean_cell(r.get("Run"))
        if rid is None:
            continue
        # allow "8" or "8.0" from Excel exports
        try:
            if int(float(rid)) == int(row_id):
                return r
        except ValueError:
            if rid == str(row_id):
                return r
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Run id from the TestMatrix.csv Run column",
    )
    parser.add_argument(
        "--csv",
        default=CSV_PATH,
    ...
    row = find_row(rows, args.row)
    if row is None:
        print("ERROR: no Run ==", args.row, "in", args.csv, file=sys.stderr)
        sys.exit(1)
    ...
    lines.append("# row=%d  Name=%s  Goal=%s" % (args.row, name, goal))
